...
bool RTMPAppProtocolHandler::ProcessInvokeConnect(BaseRTMPProtocol *pFrom,
	...
	//X. URL Token Auth by Madhawa
	string myTokenKey = "";
	myTokenKey = parts[1];
	vector<string> myparts;

	split(myTokenKey, "/", myparts);
	myTokenKey = myparts[0];

       if (_configuration["sharedKey"] != "") {
		//string sharedkey = "mykey";
		string sharedkey = _configuration["sharedKey"];
		IOHandler *pHandler=pFrom->GetIOHandler();
	       if(pHandler==NULL) 
		        return false; 
	       if(pHandler->GetType()!= IOHT_TCP_CARRIER) 
		        return false;
	       string peerAddress=((TCPCarrier *)pHandler)->GetFarEndpointAddressIp();
	       uint16_t peerPort=((TCPCarrier *)pHandler)->GetFarEndpointPort();
        
	       string md5Code = md5(peerAddress+sharedkey, true);
		
		INFO("Requested URL Token-Key is \'%s\'", STR((string) myTokenKey));
		INFO("IP + Shared Key is \'%s\'", STR((string) peerAddress+sharedkey));
		INFO("Calculated URL Token-Key is \'%s\'", STR((string) md5Code));
		INFO("Peer IP is \'%s\'", STR((string) peerAddress));
		INFO("Peer Port is \'%i\'", peerPort);

		if(myTokenKey !=md5Code) {
			FATAL("Invalid Token-Key");
			return false;
		}
      }
